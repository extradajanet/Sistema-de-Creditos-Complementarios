...
const imagenes = import.meta.glob("../../images/*.{png,jpg,jpeg,gif}", {
  eager: true,
});
...
export default function ActividadesList() {
  ...
  const obtenerCarreraIdsDesdeNombres = (nombres) => {
    ...
    return todasLasCarreras
      .filter((carrera) =>
        nombresNormalizados.includes(normalizar(carrera.nombre))
      )
  ...
  const cargarAlumnosInscritos = (idActividad) => {
    fetch(
      `https://localhost:7238/api/AlumnoActividad/alumnos-inscritos/${idActividad}`,
      {
        headers: {
      ...
      .then((data) => {
        ...
        setCanFinalize(
          data.every((a) => [4, 5].includes(a.estadoAlumnoActividad))
        );
      })
      .catch((error) => {
  ...
  const acreditarAlumno = (alumnoId, actividadId) => {
    fetch(
      `https://localhost:7238/api/AlumnoActividad/${alumnoId}/${actividadId}`,
      {
        method: "PUT",
  ...
  const eliminarAlumno = (alumnoId, actividadId) => {
    fetch(
      ...
      .then((res) => {
        ...
        setToast({ message: "Alumno eliminado de la actividad", type: "success" });
        cargarAlumnosInscritos(actividadId);
      })
      .catch((err) => {
        console.error(err);
        setToast({ message: "Error al eliminar alumno", type: "error" });
      });
  };
  ...
  const acreditarTodos = () => {
    const solicitudes = alumnos.map((alumno) =>
      fetch(
        `https://localhost:7238/api/AlumnoActividad/${alumno.alumnoId}/${actividadSeleccionada.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(4),
  ...
  const handleUpdateActividad = () => {
    ...
    const fechaInicioISO = new Date(
      fechaInicioEdit + "T00:00:00.000Z"
    ).toISOString();
    ...
    if (carrerasFinales.includes(14)) {
      carrerasFinales = todasLasCarreras
        .filter((c) => c.id !== 14)
        .map((c) => c.id);
    }
    ...
    const actividadActualizada = {
      ...
      imagenNombre:
        actividadSeleccionada.imagenNombre || "PredeterminadoCursos.png",
    ...
    if (
      [1, 2].includes(actividadActualizada.tipoActividad) &&
      typeof generoSeleccionado !== "undefined"
    ) {
    ...
    fetch(
      ...
      .then((actividadActualizadaRespuesta) => {
        const actividadActualizadaFinal =
          actividadActualizadaRespuesta || actividadActualizada;

        setActividades((prevActividades) =>
          prevActividades.map((act) =>
            act.id === actividadSeleccionada.id
              ? { ...act, ...actividadActualizadaFinal }
              : act
          )
        );
  ...
  useEffect(() => {
    ...
    fetch("/api/Actividades", { headers: { Accept: "application/json" } })
      ...
      .then(async (data) => {
        ...
        console.log(
          "Se auto‑finalizarán estas actividades:",
          vencidas.map(
            (a) => `${a.id} (${a.fechaFin.split("T")[0]} ${a.horaFin})`
          )
        );

        await Promise.all(
          vencidas.map(async (act) => {
            const payload = {
              nombre: act.nombre,
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col gap-4">
        {loading ? (
          <p className="text-center mt-10 text-black-600">
                  ...
                  <div className="flex gap-2 mt-15">
                    <button
                      ...
                      onClick={() => {
                        ...
                        const nombres = Array.isArray(actividad.carreraNombres)
                          ? actividad.carreraNombres
                          : [];
                        setCarreraIdsEdit(
                          obtenerCarreraIdsDesdeNombres(nombres)
                        );
                        setShowEdit(true);
                      }}
                    >
                    ...
                    <Modal
                      ...
                      title={
                        actividadSeleccionada?.nombre || "Editar Actividad"
                      }
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm text-gray-800">
                              ...
                              (Array.isArray(actividadSeleccionada.dias)
                                ...
                                .map((dia) => (
                                  <p key={dia}>
                                    ...
                                    &nbsp;&nbsp;&nbsp;&nbsp;
                                    {actividadSeleccionada.horaInicio?.slice(
                                      0,
                                      5
                                    )}{" "}
                                    -{" "}
                                    {actividadSeleccionada.horaFin?.slice(0, 5)}
                                  </p>
                                ))
                            ) : (
                          ...
                          <div className="col-span-1">
                            <div className="grid grid-cols-[90px_auto_auto_auto] items-center gap-2 m-2">
                              ...
                              <input
                                ...
                                value={capacidadEdit}
                                onChange={(e) =>
                                  setCapacidadEdit(Number(e.target.value))
                                }
                          ...
                          <div className="col-span-2">
                            <div className="relative mt-1 w-full">
                              ...
                              <div
                                className={`w-full border border-blue-950 rounded-lg px-3 py-2 bg-white text-sm text-gray-800 h-40 overflow-y-auto space-y-1
                              ${
                                disableCarrerasEdit
                              ...
                              >
                                {todasLasCarreras.map((carrera) => {
                                  const seleccionada = carreraIdsEdit.includes(
                                    carrera.id
                                  );
                                  return (
                                    <div
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setFechaInicioEdit(e.target.value)
                              }
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <button
                                    ...
                                    disabled={
                                      ...
                                      new Date() <
                                        new Date(actividadSeleccionada.fechaFin) // aún no termina la actividad
                                    }
                                  >
                                    <Check
                                      className={`rounded p-1 w-6 h-6 cursor-pointer ${
                                        alumno.estadoAlumnoActividad === 4
                                          ...
                                          : new Date() <
                                            new Date(
                                              actividadSeleccionada.fechaFin
                                            )
                                  ...
                                  <button
                                    ...
                                    } // deshabilitado si ya está no acreditado
                                  >
                                    <X
                                      className={`rounded p-1 w-6 h-6 cursor-pointer ${
                                        alumno.estadoAlumnoActividad === 5
                                  ...
                                  <button
                                    title="Eliminar"
                                    onClick={() => {
                                      setAlumnoAEliminar(alumno.alumnoId);
                          ...
                          <button
                            ...
                            onClick={async () => {
                              const payload = {
                                ...
                                estadoActividad: 3,
                                imagenNombre:
                                  actividadSeleccionada.imagenNombre,
                                departamentoId:
                                  actividadSeleccionada.departamentoId,
                                carreraIds:
                                  actividadSeleccionada.carreraIds || [],
                    ...
                    {showConfirmModal && (
                      <Modal
                        ...
                        title="Confirmación"
                        className="w-[500px] h-[250px] max-w-full border-4 bg-[#001F54] text-white custom-text font-semibold"
                        closeButtonClassName="text-white"
                      >
                        <div className="text-center text-white font-medium mt-4">
                          ¿Estás seguro de eliminar al alumno de esta actividad?
                        </div>

                        <div className="flex justify-center gap-6 mt-8">
                          <button
                            className="px-4 py-2 rounded-md font-bold border-2 border-white text-white cursor-pointer"
                            onClick={async () => {
                              if (alumnoAEliminar) {
                          ...
                          <button
                            className="border-2 border-white text-white cursor-pointer px-4 py-2 rounded-md font-bold"
                            onClick={() => {
                              setShowConfirmModal(false);
                              setAlumnoAEliminar(null);
                            }}
