...
const calcularEstado = (actividad) => {
  const hoy = new Date();
  ...
  if (hoy < inicio) return 1; // Activo
  if (hoy >= inicio && hoy <= fin) return 2; // En Progreso
  return 3; // Finalizado
};

const verificarYActualizarEstado = async (actividad) => {
  const nuevoEstado = calcularEstado(actividad);

  if (actividad.estadoActividad !== nuevoEstado) {
    try {
      ...
      console.log(
        `Actividad ${actividad.id} actualizada a estado ${nuevoEstado}`
      );
    } catch (error) {
...
export default function ActividadesList() {
  ...
  useEffect(() => {
    ...
    const cargarYActualizarActividades = async () => {
      try {
        ...
        // Verifica y actualiza estados si es necesario
        await Promise.all(data.map(verificarYActualizarEstado));

        // Una vez hecho, vuelve a pedir la lista actualizada
        const resFinal = await fetch("/api/Actividades", {
          headers: { Accept: "application/json" },
        });

        if (!resFinal.ok) throw new Error("Error: " + resFinal.status);

        const actividadesActualizadas = await resFinal.json();
        ...
        <div className="relative">
          ...
          {mostrarFiltro && (
            <div className="absolute right-0 mt-2 bg-white border border-blue-950 rounded-xl shadow-lg z-10 p-2 w-48 space-y-3">
              ...
              {/* Si la categoría es Extraescolar, muestra estos filtros adicionales */}
