...
namespace SistemaCreditosComplementarios.Core.Services.AlumnoActividadServices
{
    ...
    public class AlumnoActividadService : IAlumnoActividadService
    {
        ...
        // Reemplazar el método AddAsync para corregir el error CS0029
        public async Task<AlumnoActividadDto> AddAsync(AlumnoActividadCreateDto alumnoActividadCreateDto)
        {
            ...
            var actividad = await _alumnoActividadRepository.GetByIdAsync(alumnoActividadCreateDto.AlumnoId, alumnoActividadCreateDto.ActividadId);
            if (actividad == null)
            {
                throw new Exception("Actividad del alumno no encontrada después de agregar.");
            }
            return new AlumnoActividadDto
            {
